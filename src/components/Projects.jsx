...
const projects = [
    {
    title: 'Yonny QA',
    description: 'It is an AI-powered QA orchestrator that automates test generation: it reads user stories from Jira, creates BDD scenarios with Google Gemini, produces automation scripts in Pytest, and generates Allure reports.',
    image: '/imgs/yonnyqa.png',
    tags: ['Java Script', 'Python', 'Node Js'],
    liveUrl: 'https://yonnyqa.onrender.com',
    cat: 'fullstack',
    featured: true,
  },
  {
    ...
    tags: ['Next.js', 'React', 'Tailwind', 'Java Script'],
  ...
  },
  {
    ...
    tags: ['React', 'Java Script', 'CSS', 'firebase'],
...
const ProjectCard = ({ project, featured = false }) => {
  ...
  const rotateX = useSpring(useTransform(mvY, [-0.5, 0.5], [7, -7]), { stiffness: 200, damping: 20 })
  const rotateY = useSpring(useTransform(mvX, [-0.5, 0.5], [-7, 7]), { stiffness: 200, damping: 20 })
  ...
  const handleMove = (e) => {
    ...
    mvX.set(px - 0.5)
    mvY.set(py - 0.5)
    setSpot({ x: px * 100, y: py * 100, show: true })
  }
  const handleLeave = () => {
    mvX.set(0)
    mvY.set(0)
  ...
  <motion.div
    ...
    className={`group card overflow-hidden !p-0 relative ${featured ? 'sm:col-span-2' : ''}`}
  >
    <div
      className="pointer-events-none absolute inset-0 z-20 rounded-2xl transition-opacity duration-300"
      style={{
        opacity: spot.show ? 1 : 0,
        background: `radial-gradient(420px circle at ${spot.x}% ${spot.y}%, rgba(59,130,246,0.14), transparent 70%)`,
      }}
    />
              ...
              <button
                ...
                className={`px-4 py-1.5 rounded-full text-xs font-medium transition-all duration-300 ${
                  active === f.value
                    ...
                    : 'text-text-muted border border-overlay/[0.06] hover:text-text-secondary hover:border-overlay/[0.12]'
                }`}
              >
                {f.label}
              </button>
