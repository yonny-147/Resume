...
import {
  SiReact, SiNextdotjs, SiJavascript, SiTypescript, SiHtml5, SiCss, SiTailwindcss,
  SiNodedotjs, SiNestjs, SiExpress, SiPhp, SiLaravel, SiMysql, SiFirebase,
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col items-center justify-center gap-3 p-5 rounded-2xl
                 border border-overlay/[0.05] bg-overlay/[0.02]
                 hover:border-overlay/[0.12] hover:bg-overlay/[0.05]
  ...
  return (
    <section aria-label="Skills" className="relative" ref={ref}>
      <div className="section-container">
          ...
          {categories.map((cat) => (
            <button
              ...
              className={`relative px-4 py-2 rounded-full text-xs font-medium transition-all duration-300 cursor-pointer
                ${active === cat.id
                  ? 'text-primary'
                  : 'text-text-muted hover:text-text-secondary border border-overlay/[0.06] hover:border-overlay/[0.12] bg-overlay/[0.02]'
                }`}
            >
              {active === cat.id && (
                <motion.div
        ...
        {/* Tech marquee */}
        <div className="relative mt-10 overflow-hidden marquee-mask">
            ...
            {[...allSkills, ...allSkills].map((s, i) => {
              ...
              return (
                <div key={i} className="flex shrink-0 items-center gap-2">
                  ...
                  <span className="whitespace-nowrap text-sm font-medium text-text-muted">{s.name}</span>
                </div>
              )
            })}
          </div>
        ...
        >
          <div className="h-px flex-1 bg-overlay/[0.04]" />
